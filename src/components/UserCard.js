...
function UserCard(props) {
  ...
  return (
    <Card
      ...
      style={styles.cardContainer(isMobile)}>
      <Box sx={{ display: 'flex', flexDirection: 'column' }}>
        <CardContent sx={{ flex: '1 0 auto' }}>
          <Typography
            component="div"
            variant="h5"
            style={styles.header(isMobile)}>
            Live From Space
          </Typography>
          <Typography
            variant="subtitle1"
...
const styles = {
  cardContainer: (isMobile) => ({
    ...
    height: isMobile ? '125px' : '100%',
    margin: isMobile ? '10px 0' : '5px',
  }),
  cardImage: (isMobile) => ({
    width: isMobile ? '125px' : '40%',
  }),
  header: (isMobile) => ({
